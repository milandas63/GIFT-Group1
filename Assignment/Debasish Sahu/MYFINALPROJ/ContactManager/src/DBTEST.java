...
public class DBTEST {
    ...
    public static void CONMENU()throws Exception{                 
        ...
        if(CH==1){                                                  //AS ADDING,MODIFYING/UPDATING,DELETING,DISPLAYING 
            ...
            {                                                           //FROM THIS TO 
                ...
                while(RS.next()){                                       //
                    for(int i=1;i<=RSMD.getColumnCount();i++){          //
                        ...
                        if(COLARR[i-1]<((RS.getString(i)==null)?4:(RS.getString(i).toString().length()))){     //
            ...
            while(RS.next()){                                                       //THIS TO
                for(int i=1;i<=RSMD.getColumnCount();i++){                          //
                    int Tmax=COLARR[i-1].intValue();                                //
                    System.out.print("| "+(PADR(Tmax,((RS.getString(i)==null)?"NULL":RS.getString(i)),' '))+" ");  //PRINTING THE COLUMN ELEMENTS
    ...
    public static void main(String[] args)throws Exception{
        ...
        CNCT=DriverManager.getConnection("jdbc:mysql://localhost:3306/contactdbT0", "root", "");
